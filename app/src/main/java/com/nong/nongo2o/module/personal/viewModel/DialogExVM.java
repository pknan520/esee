...
public class DialogExVM implements ViewModel {
    ...
    public interface DialogExListener {
        void confirmClick(String exName, String exNumber, String remark);
    }
    ...
    private void confirm() {
        if (TextUtils.isEmpty(exName.get()) || TextUtils.isEmpty(exNumber.get())) {
            Toast.makeText(context, "请输入完整的快递信息", Toast.LENGTH_SHORT).show();
            return;
        }

        if (listener != null) {
            listener.confirmClick(exName.get(), exNumber.get(), remark.get());
        }
